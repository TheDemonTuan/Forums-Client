...
import { LogoutAuthResponse, logoutAuth } from "@/lib/api/authApi";
...
export const UserNav = memo(() => {
  ...
  useEffect(() => {
    if (logoutIsPending) toast.loading("Logging out...");
    else toast.dismiss();
  }, [logoutIsPending]);
  ...
  const { authData, authIsLoading, authIsSuccess } = useAuth();

  if (authIsLoading) return <Skeleton className="h-12 w-12 rounded-full bg-forum_gray" />;
  else if (!authIsSuccess || !authData) return null;
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56" align="end" forceMount>
          ...
          <DropdownMenuItem onClick={handleLogout} disabled={logoutIsPending}>
            {logoutIsPending ? "Logging out..." : "Logout"}

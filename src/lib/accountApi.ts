...
export const passwordAccount = async (body: PasswordAccountBody) => http.put<AuthResponse>("account/password", body).then((res) => res.data);
...
export const sessionAccountRevoke = async (body: SessionRevokeBody) => http.delete(`account/session/revoke/${body?.id}`).then((res) => res.data);

export interface SessionRevokeAllResponse {
	message: string;
}

export const sessionAccountRevokeAll = async () => http.delete(`account/session/revoke/all`).then((res) => res.data);
